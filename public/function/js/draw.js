...
var draw_graph = function(graphType,obj){	
	...
	var mousedown = function(e){
		...
		e=e||window.event;
		startX = (e.clientX - document.getElementById("canvas").getBoundingClientRect().left)/square;
		startY = (e.clientY - document.getElementById("canvas").getBoundingClientRect().top)/square;
		//console.log(e.clientX+" "+e.clientY);
		context_bak.moveTo(startX ,startY );
		canDraw = true;
		//console.log($(canvas).offset().left+" "+$(canvas).offset().top+" "+$(canvas_show).offset().left+" "+$(canvas_show).offset().top);
	...
	var mouseup = function(e){
		...
		if(graphType!='rubber'){
			...
			image.onload = function(){
				//context.drawImage(image , 0 ,0 , image.width , image.height , 0 ,0 , $('#canvas').attr('width') , $('#canvas').attr('height'));
				context.drawImage(image,0,0,image.width,image.height,0,0,canvasWidth,canvasHeight);
				//console.log($('#canvas').attr('width')+"  "+$('#canvas').attr('height'));
				clearContext();
				saveImageToAry();
			}
			var x = (e.clientX   -document.getElementById("canvas").getBoundingClientRect().left)/square;
			var y = (e.clientY  - document.getElementById("canvas").getBoundingClientRect().top)/square;
			context.beginPath();
			context.moveTo(x ,y );
	...
	var  mousemove = function(e){
		...
		//画笔
			...
			if(canDraw){
				context_bak.lineTo((e.clientX   -document.getElementById("canvas").getBoundingClientRect().left)/square ,(e.clientY  - document.getElementById("canvas").getBoundingClientRect().top)/square);
				context_bak.stroke();						
			}
		//圆 未画得时候 出现一个小圆
		}else if(graphType == 'circle'){						
