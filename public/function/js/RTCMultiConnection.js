...
(function() {
    //使用navigator.userAgent来判断浏览器类型,如果没有浏览器则退出
    var whereApple = navigator.userAgent.indexOf('hone');
    if (whereApple>-1){
    ...
    //location.href表示当前页面的链接，将URL中的一些字符删除
    window.RMCDefaultChannel = location.href.replace(/\/|:|#|%|\.|\[|\]/g, '');

    // www.RTCMultiConnection.org/docs/constructor/
    window.RTCMultiConnection = function(channel) {//相当于function RTCMultiConnection(channel){}
        ...
        function joinSession(session, joinAs) {
            ...
            if (!rtcMultiSession) {//rtcMultiSession== null时执行
        ...
        function receive(data, userid, extra) {
            ...
            if (data.last) {
                ...
                if (message.preRecordedMediaChunk) {
                    ...
                    connection.onacceptMessage(message);
                }else connection.onmessage(e);
    ...
    function capturePartOfScreen(args) {
        ...
        //var canvasPdf = document.getElementById(element);
        //args.callback(canvasPdf.toDataURL("image/png"));
        html2canvas(element, {
    ...
    function setDefaults(connection) {
        ...
        connection.onacceptMessage = function(e){
        ...
        connection.OwnShareScreen = function(args) {
            var lastScreenshot = '';
            //if(connection)
            function mypartOfScreenCapturer() {
                ...
                capturePartOfScreen({
                    ...
                    callback: function (screenshot) {
                        ...
                        !args.once && setTimeout(mypartOfScreenCapturer, args.interval || 200);
                    }
                });

            }
            mypartOfScreenCapturer();
        }
