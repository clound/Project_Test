...
function initReliableSignaler(connection, socketURL) {
    ...
    function addFriendListener(socket) {
        ...
        //  有人上线后，刷新好友在线状态
        socket.on('_update_friends_state', function (friend) {
            socket.emit('update_friends_state', friend);
        });
        //  解除好友关系后，刷新好友状态
        socket.on('_update_your_friendsState', function (user) {
            if (user === globalclientUser) {
                getfriedname(globalclientUser);
            }
        });
        //  解除好友关系后，刷新sessionRooms的状态
        socket.on('_update_your_sessionRooms', function (user) {
            if (user === globalclientUser) {
                socket.emit('update_getSessions');
            }
        });
    };
    ...
    function roomEventListener(socket) {
        ...
        socket.on('_inviteIntoRoom', function (roomInfo) {
            if(roomInfo.friend === globalclientUser) {
                //  发送信号，刷新可加入列表
                socket.emit('update_push_invite_room');
                if(alertBox_JoinRoom(roomInfo.owner, roomInfo.sessionid)) {
                    joinAMeeting(roomInfo, socket);
                }
            }
        });
        //  有人退出/加入房间 更新状态
        socket.on('_update_RoomUserState', function (owner) {
            if(owner === globalclientUser) {
                socket.emit('update_RoomUserState');
            }
        });
    }

    function alertBox_JoinRoom(owner, session) {
    //利用对话框返回的值 （true 或者 false）
        if(confirm(owner + "想要邀请你进入会议" + session))
        {//如果是true ，那么就是同意加好友
            //alert("同意了，马上要进入会议！");
            return true;
        }
    ...
    // 加入一个会议
    function joinAMeeting(roomInfo, socket) {
        if(FlagOnMeeting) {
            alert('已经在一个会议室，不能同时参加多个会议！');
            return;
        }
        ...
        socket.emit("get-session-info", roomInfo.sessionid, function (sessionid) {
            if (sessionid === roomInfo.sessionid) {
                ...
                alert("成功进入房间！" + connection.channel + " " + connection.userid + " " + connection.sessionid);
                FlagOnMeeting = true;
                globalRoomSession = roomInfo.sessionid;
                socket.emit('success-join-room', roomInfo);
                document.getElementById('out_room').disabled = false;
    ...
    /************************  获取群组信息***********************/
    function get_some_push_msg(socket) {
        //  会议的解散信息
        socket.on('get_disMissMeeting', function (meeting, user, callback) {
            if(user && user !== globalclientUser ) {
                return;
            }
            return alertBox_disMissMeeting(meeting) ? callback(meeting, globalclientUser) : null;
        });

        function alertBox_disMissMeeting(meeting) {
            if(confirm(meeting.owner+'将会议'+meeting.sessionId+"解散! 确认不再通知！")) {
                return true;
            } else {
                return false;
            }
    ...
    return {
        ...
        joinAMeeting: function(roomInfo) {
            joinAMeeting(roomInfo, socket);
        }
    };
}
